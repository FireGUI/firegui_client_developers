...
$PHPTHUMB_CONFIG['cache_directory'] = $PHPTHUMB_CONFIG['document_root'].'/class/cache/';      // set the cache directory to an absolute directory for all source images
...
if (!is_dir($PHPTHUMB_CONFIG['cache_directory'])) {
    mkdir($PHPTHUMB_CONFIG['cache_directory'], 0755, true);
}
...
$PHPTHUMB_CONFIG['cache_maxfiles'] = 50000;             // delete least-recently-accessed cached thumbnails when more than [200] cached files are present (value is maximum number of cached files to keep)
...
array_pop($current_path_expl);
$current_path = implode('/', $current_path_expl);
$no_image = "$current_path/images/no_image.png";
$PHPTHUMB_CONFIG['error_message_image_default'] = $no_image;       // Set this to the name of a generic error image (e.g. '/images/error.png') that you want displayed in place of any error message that may occur. This setting is overridden by the 'err' parameter, which does the same thing.
