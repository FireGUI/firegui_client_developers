...
$(document).ready(function () {
  $('.js-bulk-action > option[value=""]')
    .attr("selected", "selected")
    .trigger("change");
  $("input:checkbox.js_bulk_check,.js-bulk-select-all")
    .removeAttr("checked")
    .trigger("change");
  $.uniform.update();
  $(".js-bulk-select-all").on("click", function () {
    var grid_container = $(this)
      .closest("table.table")
      .closest(".grid,.tab-pane");

    if ($(this).is(":checked")) {
      $('input[type="checkbox"].js_bulk_check', grid_container)
  ...
  $(".js-bulk-action").on("change", function () {
    ...
    if ($(this).val() != "") {
      ...
      if ($(this).val() == "bulk_edit") {
        ...
        loadModal(
          base_url + "get_ajax/modal_form/" + form_id,
          data_post,
          null,
          "POST"
        );
        ...
        var r = confirm(
          "Confermi di voler eliminare " + chkbx_ids.length + " righe?"
        );
        if (r == true) {
          var url =
            base_url + "db_ajax/generic_delete/" + $(this).data("entity-name");
          var data_post = [];
