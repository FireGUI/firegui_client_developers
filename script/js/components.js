...
function initComponents(container) {
  ...
  forms.each(function () {
    ...
    var btn = $(".js_filter_form_add_row", form).on("click", function () {
      ...
      $("input, select", obj).each(function () {
        var name =
          "conditions[" + size + "][" + $(this).attr("data-name") + "]";
  ...
  try {
    $(
      ".js_multiselect:not(.select2-offscreen):not(.select2-container)",
      container
    ).each(function () {
      var that = $(this);
  ...
  var fieldsSources = [];
  $('[data-source-field]:not([data-source-field=""])', container).each(
    function () {
      ...
      jqField.on("change", function () {
        ...
        var isNormalSelect =
          jsMultiselect.is("select") && !$(this).attr("multiple");
    ...
    success: function (data) {
      ...
      $(".modal", modalContainer)
        ...
        .on("shown.bs.modal", function (e) {
          ...
          $(".modal", modalContainer).data(
            "bs.modal"
          ).askConfirmationOnClose = false;

          if ($("form", modalContainer).length > 0) {
            $("input:not([type=hidden]), select, textarea", modalContainer).on(
              "change",
              function () {
                $(".modal", modalContainer).data(
                  "bs.modal"
                ).askConfirmationOnClose = true;
              }
            );
          }
        })
        .on("hide.bs.modal", function (e) {
          ...
          if ($(".modal", modalContainer).is(e.target)) {
            var askConfirmationOnClose = $(".modal", modalContainer).data(
              "bs.modal"
...
function openCreationForm(formId, entity, onSuccess) {
  $.getJSON(base_url + "get_ajax/getLastRecord", { entity: entity }, function (
    json
  ) {
    ...
    $.fancybox.open({
      ...
      beforeClose: function () {
        $.getJSON(
          base_url + "get_ajax/getLastRecord",
          { entity: entity },
          function (json) {
            if (
              json.status === 0 &&
              json.data.id &&
              currentLastId != json.data.id
            ) {
