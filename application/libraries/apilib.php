...
class Apilib {
    ...
    private function processData($entity, array &$dati, $editMode=false, $value_id = null) {
        ...
        foreach ($fields as $field) {
            ...
            // Inserisci la regola required per i campi che la richiedono (una password è required solo se sto creando il record per la prima volta)
            if ($field['fields_required'] === 't' && !$field['fields_default'] && ($field['fields_draw_html_type']!=='input_password' || !$editMode)) {
                $rule[] = 'required';
            }
        ...
        foreach ($fields as $field) {
            ...
            switch ($html_type) {
                ...
                case 'input_password':
                    ...
                    break;
                case 'map':
                    $dati[$field['fields_name']] = str_replace(',', ';', $dati[$field['fields_name']]);
