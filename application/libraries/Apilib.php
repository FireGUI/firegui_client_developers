...
class Apilib {
    ...
    public function edit($entity=null, $id=null, $data=null, $returnRecord = true) {
        ...
        if($this->processData($entity, $_data, true, $id)) {
            ...
            $newData = $this->runDataProcessing($entity, 'save', 
                array_merge(
...
//                    (!$editMode && $value === '' && !$isRequired)
         ...
         */
        $processed_data_1 = $this->runDataProcessing($entity_data['entity_id'], "pre-{$mode}", ['post' => $dati, 'value_id' => $value_id]); // Pre-process specifico
