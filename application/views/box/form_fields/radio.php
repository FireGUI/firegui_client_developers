        ...
        <div class="radio-list" style="margin-left: 6px;">
            <?php if(!empty($field['support_data'])): ?>
                ...
                <?php foreach ($field['support_data'] as $value): ?>
                    <label class="radio">
                        <input class="<?php echo $class ?>" type="radio" value="<?php echo $value[$field['fields_ref']."_id"]; ?>" <?php if($value[$field['fields_ref']."_id"] == $selectedValue) echo 'checked'; ?> name="<?php echo $field['fields_name']; ?>" <?php if($field['fields_draw_onclick']) echo 'onclick="'.$field['fields_draw_onclick'].'"' ?> />
