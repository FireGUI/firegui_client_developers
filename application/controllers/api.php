...
class Api extends CI_Controller {
    ...
    public function entity_list() {
        $tables = $this->apilib->entity_list();
        echo '<pre>'.print_r($tables, true).'</pre>';
    }
    
    public function support_list() {
        $tables = $this->apilib->support_list();
        echo '<pre>'.print_r($tables, true).'</pre>';
    }
    ...
    public function login($entity=null) {
        ...
        try {
            ...
            $arrayOutput = $this->apilib->search($entity, $data, 1);
            
            $this->logAction(__FUNCTION__, func_get_args());
            $output = $this->apilib->runDataProcessing($entity, 'login', array_shift($arrayOutput));
