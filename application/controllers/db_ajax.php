...
class Db_ajax extends CI_Controller {
    ...
    public function save_form($form_id, $edit = false, $value_id = null) {
         ...
         */
        $this->db->trans_start();
        $form = $this->db->from('forms')->join('entity', 'forms_entity_id=entity_id', 'left')->where('forms_id', $form_id)->get()->row_array();
        $form['fields'] = $this->db->
                        from('forms_fields')->
                        join('fields', 'forms_fields_fields_id=fields_id', 'left')->
                        join('fields_draw', 'fields_draw_fields_id=fields_id', 'left')->
                        where('forms_fields_forms_id', $form_id)->get()->result_array();
        ...
        if ($x) {
            ...
            if($edit) {
                $dati[$form['entity_name'] . "_id"] = $insert_id;
                $this->datab->run_post_process($form['entity_id'], 'update', array('new' => $dati, 'old' => $old_data, 'diff' => array_diff_assoc($dati, $old_data)));
            } else {
                $dati[$form['entity_name'] . "_id"] = $insert_id;
                $this->datab->run_post_process($form['entity_id'], 'insert', $dati);
            }
            ...
            $replaceFrom = array_map(function($key) { return '{' . $key . '}'; }, array_keys($dati));
            ...
            if ($committed) {
                echo json_encode(array('status' => $status, 'txt' => $message, 'close_modals' => 1));
            } else {
