    <?php if (!empty($dati['results'])) : ?>

        <section class="content-header">
            <h1>
                <?php e('Trash'); ?>
                <small><?php e('If an item is displayed on this page it means that it has been deleted'); ?></small>
            </h1>
        </section>
                                ...
                                <table class="table table-condensed table-bordered table-hover table-scrollable table-scrollable-borderless js_search_datatable">
                                    ...
                                    <tbody>
                                        <?php foreach ($entity_result['data'] as $data) : ?>
                                            <tr>
                                                <td><?php
                                                    echo $data[$entity_result['entity']['entity_name'] . '_id']; ?></td>
                                                <?php foreach ($fields as $i => $field) : ?>
                                                    <td>
                                                        <?php
                                                ...
                                                <td>
                                                    <div class="action-list">
                                                        <a class="js-action_button btn  js_confirm_button js_link_ajax" href="<?php echo base_url('db_ajax/switch_bool/' . $entity_result['entity']['entity_soft_delete_field'] . '/' . $data[$entity_result['entity']['entity_name'] . '_id']); ?>" data-confirm-text="Sei sicuro di voler ripristinare questo record?" data-toggle="tooltip" data-original-title="" title="">
    ...
    <?php else : ?>
        ...
        <div class="col-md-12">
            <h4 class="box-title"><?php e('Trash'); ?> <small><?php e('no results found'); ?></small></h4>
        </div>
